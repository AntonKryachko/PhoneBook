...
trait lLiquibaseComponent

class lLiquibaseComponentImpl @Inject()(lifecycle: ApplicationLifecycle, db: Database) extends lLiquibaseComponent {
...
class lLiquibaseModule extends AbstractModule {
  override def configure(): Unit = {
    bind(classOf[lLiquibaseComponent]).to(classOf[lLiquibaseComponentImpl]).asEagerSingleton()
  }
}